    ...
    if all_trans.empty:
        ...
        if filtered_df.empty:
            ...
            **የ {selected_month} ወር የስቶክ እንቅስቃሴ ማጠቃለያ ትንተና፦**
            ...
            2. **ከፍተኛ ገቢ የተደረገበት ዕቃ፦** በወሩ ውስጥ ከፍተኛ መጠን ያለው ገቢ የተመዘገበበት ዕቃ **{top_in_row[name_col] if top_in_row is not None else 'N/A'}** (ኮድ: `{top_in_row['clean_code'] if top_in_row is not None else ''}`) ሲሆን፣ አጠቃላይ የገባው ብዛት **{top_in_row['Total_Qty_In'] if top_in_row is not None else 0:,.0f}** ነው።

            3. **ከፍተኛ ወጪ የተደረገበት ዕቃ፦** በወሩ በብዛት ለምርት/ለስራ ወጪ የተደረገው ዕቃ **{top_out_row[name_col] if top_out_row is not None else 'N/A'}** (ኮድ: `{top_out_row['clean_code'] if top_out_row is not None else ''}`) ሲሆን፣ አጠቃላይ የወጣው ብዛት **{top_out_row['Total_Qty_Out'] if top_out_row is not None else 0:,.0f}** ነው።
            ...
            with col2:
                ...
                st.download_button("📥 Download Report as CSV", data=csv_m, file_name=f"Monthly_Report_{selected_month}.csv", mime="text/csv")
